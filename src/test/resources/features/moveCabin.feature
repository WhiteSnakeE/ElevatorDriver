Feature: move cabin
  ...
  Scenario: calling a cabins from different houses with all free shafts in first house and one busy shaft in second house and direction "UPWARDS"
    ...
    Then commands should have be invoked for shaft 1 in house 0: MoveCabinCommand, StopEngineCommand, OpenDoorCommand, CloseDoorCommand for floor 5
    And shaft 1 in house 0 has cabin position 5
    And commands should have be invoked for shaft 0 in house 1: MoveCabinCommand, StopEngineCommand, OpenDoorCommand, CloseDoorCommand for floors 8, 9
